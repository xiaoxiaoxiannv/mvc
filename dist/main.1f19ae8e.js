...
var n = localStorage.getItem("n"); //得到n

$number.text(n || 100); //有的话就是n，没有就是100

$button1.on("click", function () {
  var n = parseInt($number.text()); //设置一个n是把number转化为数字的
...
$tabBar.on("click", "li", function (e) {
  //监听父元素下子元素的事件（事件委托）
  var $li = (0, _jquery.default)(e.currentTarget); //找到那个子元素

  $li.addClass("selected") //添加selected类
  .siblings() //找到兄弟
  .removeClass("selected"); //删除selected

  var index = $li.index();
  $tabContent.children() //找到孩子
  .eq(index) //找到对应的下标元素
